...
from lib.task.nlp import make_graph_from_cooc, SIMILARITY_BENCHMARKS
...
def run_master(args):
    ...
    if not os.path.exists(model_init_path) or args.force_create_graph:
        if args.init == 'glove':
            ...
            graph_embedding = make_graph_from_cooc(
                dataset.cooc_matrix, knn_edges=args.knn_edges, random_edges=args.random_edges, max_length=20, n_jobs=-1,
                soft=args.soft, k_nearest=0, sparse=args.sparse, GraphEmbeddingClass=ParallelGraphEmbedding,
                cooc_mode=args.cooc_prediction_mode,
                verbose=True)
            if args.cooc_prediction_mode == 'DOT_PRODUCT':
                null_vertex = graph_embedding.num_vertices - 1
            else:
                null_vertex = None
            model = GloveModel(
                graph_embedding, token_to_ix=corpora.dictionary, null_vertex=null_vertex,
                cooc_mode=getattr(GloveModel.COOCMode, args.cooc_prediction_mode)
            )
            with torch.no_grad():
                if args.cooc_prediction_mode == 'DOT_PRODUCT':
                    model.biases.weight.data[:model.null_vertex, 0] = 0.
                elif args.cooc_prediction_mode in ('DISTANCE', 'DISTANCE_SQUARED'):
                    frequencies = np.asarray(dataset.cooc_matrix.sum(0))
                    model.biases.weight.data[:, 0] = torch.from_numpy(np.log(frequencies))
                else:
                    raise ValueError
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init', choices=('glove', 'rank'), default='glove')
